...
int do_hist_distDATA(){
  TFile *myFile = new TFile("diphoFile_DATA.root","RECREATE");
   chain->Add("diphoton_part*.root");
  
TString baseCut = "(abs(pholead_eta)<=2.5)&&(!((abs(pholead_eta)>1.4442)&&(abs(pholead_eta)<1.566)))&&(abs(pholead_eta)<=2.5)&&(!((abs(pholead_eta)>1.4442)&&(abs(pholead_eta)<1.566)))&&pholead_hoe<0.05&&photrail_hoe<0.05&&pholead_pt>30&&photrail_pt>30";
